...
create table users (
  ...
  name text not null,
  roles text[] not null default '{}',        -- subset of the 6 role values below
  open_ef_reviews int not null default 0,
  open_ef_approvals int not null default 0,
  open_ef_blocked_items int not null default 0
);
comment on column users.roles is 'Values: Expert EF Reviewer, Standard EF Reviewer, EF Approver, EF Data Steward, Project Owner, Project Reviewer';
...
create table ef_proposals (
  ...
  status text not null default 'E1-Draft',  -- E1-Draft | E2-Review | E3-Approval | E4-Ingestion | E5-Blocked | E6-Cancelled
  ...
  data_steward_id text references users(user_id), -- auto-assigned on Mark as Blocked (§7.11); persists after Resume like reviewer/approver do
  blocked_from_status text,            -- remembers which stage to return to after Blocked (renamed from on_hold_from_status)
  blocked_reason text,                 -- mandatory dropdown reason captured on Mark as Blocked
  blocked_comment text,                -- mandatory free-text comment captured on Mark as Blocked
  snow_ticket_id text,                 -- self-generated by EF Tracker at E3->E4 (§7.10) - stand-in for a real SNOW API
  snow_ticket_status text,             -- one of SNOW_STATUSES; only changes via the manual SNOW Simulator
  snow_ticket_raised_at timestamptz,
